function myFunction() {
    /*
    The main purpose of this function is to parse my own json data into a combobox as category titles.
    When the user selects an option from the combobox then all of the sub-sections are updated
    accordingly in real time. One thing I have noticed is that local json api data does not require
    unsafe scripts to be loaded by third party json api data does.
    */
    let arr; //array to contain parsed json data
    let xmlhttp = new XMLHttpRequest(); //request to fetch json data
    let aboutTitle = document.getElementById("aboutPageSub"); //inner combobox titles

    //url for my json data
    let url = "https://raw.githubusercontent.com/cdavis208/Assignment-2-Website-Project-Code/master/resources/data/data.json";

    // gathers the data from the url provided over HTTP
    ...
    //sending a non https request, url is HTTPS so it can be loaded safely
    xmlhttp.send();


    /*
    This function parses the json data into an array, and then fills the combobox from the 
    array data using a for loop
    */
    function gatherData(arr) {

        this.arr = arr; //super arr

        let feedJSONdata = document.getElementById("sel"); //item to be selected in combobox
        let selectedVal = feedJSONdata.selectedIndex; //return selected item in combobox

        let comboBoxData = ""; //string to append the new fed data
        let i; //variable for the for loop

        /*
        This for loop populates the combobox with the json data from the array parsed.
        i increases for the size of the array of json data.
        */
        for (i = 0; i < arr.aboutPage.length; i++) {

            comboBoxData += `<option>${arr.aboutPage[i].title}</option>`;
            aboutTitle.innerHTML = arr.aboutPage[0].title; //title defaults to first entry


        }
        feedJSONdata.innerHTML = comboBoxData; //update combobox with the fed data
    }

}

/*
This function dynamically updates the sub-sections in the main content. When the user selects
a new option from the combobox, then the javascript detects that pasting the relevant content 
for that category heading.
*/
function updateAboutTitles() {
    ...
    let subTitleData = ""; //title of selected element from combox, overwritten when changed selection in combobox
    let reason1Data = ""; //section one text, overwritten when changed selection in combobox
    let reason2Data = ""; //section two text, overwritten when changed selection in combobox
    let reason3Data = ""; //section three text, overwritten when changed selection in combobox
    let box1Text = document.getElementById("box1Txt"); //return section one from html
    let box2Text = document.getElementById("box2Txt"); //return section two from html
    let box3Text = document.getElementById("box3Txt"); //return section three from html
    let box1Img = document.getElementById("box1img"); //return section one image div from html
    let box2Img = document.getElementById("box2img"); //return section two image div from html
    let box3Img = document.getElementById("box3img"); //return section thre image div from html
    let aboutTitle = document.getElementById("aboutPageSub"); //returns the label to update title from combobox

    //first value do this and updat text accordingly
    if (selectedVal == 0) {
    ...
    //second value do this and updat text accordingly
    if (selectedVal == 1) {
    ...
    if (selectedVal == 2) {
        //third value do this and updat text accordingly
    ...
    if (selectedVal == 3) {
        //fourth value do this and updat text accordingly
