let buttonHolder = document.getElementById("button-holder"); //return the button holder from the html
let body = document.getElementById("body"); //return the body from the html
let z = 0; //old counting method if something was triggered or not
let getSearchTerm = document.getElementById("search"); //old work from when I had a search function

function myFunction() {
    /*
    This javascript file acts as the backbone to all of the logic for submarine catelogue page. When
    the user clicks a flag icon then javascript dynamically updates one of the four sections on the image
    slider. This works simiarily to the about page but on a larger scale. The design solution I came up with
    was to check for state change in a windows browser ending, if it detected what the href would append was
    correct to a submarines slider link, then the javascript would kick in.
    */

    /*
    If the window detects to have its statechanged to another image triggered from the href in html
    due to my slider logic, then the javascript will update accordingly. Setting each of the sections and
    headers to the text they should be.
    */
